...
def clean_response(text: str) -> str:
    """
    Clean LLM responses to remove markdown formatting, special tokens, and unwanted prefixes.
    """
    if not text:
        return ""
    
    # Remove special tokens like [/s]
    text = re.sub(r'\[/s\]', '', text)
    
    # Remove common markdown prefixes like "Summary:", "Answer:", etc.
    text = re.sub(r'^(Summary:|Answer:|Response:)\s*', '', text, flags=re.IGNORECASE | re.MULTILINE)
    
    # Clean up excessive newlines at start/end
    text = re.sub(r'^\n+', '', text)
    text = re.sub(r'\n+$', '', text)
    ...
    # Ensure proper sentence capitalization if starting with lowercase
...
def summarize_with_llm_from_stats(llm, stats_text: str, user_query: str) -> str:
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", 
         ...
         "Return only the summary text without any markdown formatting, headers, or special tokens."),
        ("user", 
         ...
         "Respond with just the summary text, no formatting or headers.")
    ])
    ...
    try:
        ...
        if stats_map:  # We have numerical data to summarize
            if llm:
                ...
                if use_phenomenon_prompt:
                    ...
                    response = summarize_with_llm_from_stats(llm, stats_text, user_query)
                else:
                    response = summarize_with_llm_from_stats(llm, stats_text, user_query)
            else:
                print("DEBUG: Creating enhanced simple summary from statistics (no LLM)")
                response = create_enhanced_simple_summary_from_stats(stats_text, user_query)
        ...
        if docs:
            if llm:
                ...
                try:
                    response = create_context_summary_with_llm(llm, context, user_query)
                except Exception as e:
                    print(f"WARNING: LLM context summarization failed: {e}")
                    # Fall through to basic response
                    response = f"Found relevant oceanographic profiles matching your query about {user_query}."
            else:
