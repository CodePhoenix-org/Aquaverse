...
from langchain_openai import ChatOpenAI  # Use ChatOpenAI for chat completions
...
db_uri = 'postgresql://postgres:Aashi%401234@localhost:5432/argo_db'  # Corrected URI
...
# Step 2: RAG Query with OpenRouter
print("\n=== STEP 2: Running RAG Query ===")
# Updated LLM setup with ChatOpenAI
llm = ChatOpenAI(
    model_name="mistralai/mistral-7b-instruct:free",  # Free tier model
...
# Generate SQL with refined prompt
prompt = ChatPromptTemplate.from_messages([
    ("system", "You are an ocean data expert. Return ONLY the SQL query with no explanations, comments, or extra text. Use LIKE 'YYYY-MM%' for time-based filtering if time is a string column."),
...
try:
    ...
    print("Please ensure PostgreSQL is running and the argo_profiles table exists with the correct schema. Check the URI: postgresql://postgres:Aashi%401234@localhost:5432/argo_db")

# Summarize (with fallback if SQL fails)
if 'df_result' in locals():
    summary_prompt = ChatPromptTemplate.from_messages([
        ("system", "You are an ocean data expert. Focus on ocean data, avoid hallucinations."),
