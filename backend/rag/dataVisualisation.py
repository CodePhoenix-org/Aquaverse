...
#         sql_cache_key = hashlib.md5(user_query.encode()).hexdigest()  # Unique key
#         if llm:
#             if sql_cache_key in sql_cache:
#                 sql = sql_cache[sql_cache_key]
#                 print(f"DEBUG: Using cached SQL: {sql}")
#             else:
#                 sql_prompt = create_enhanced_sql_prompt()  # Ensure this is defined
...
#                             sql
#                         )
#                         break  # Stop after first replacement to avoid over-modifying
#             try:
...
#         # Step 5: Generate consistent response
...
#                 response_text = (
#                     f"No specific data matching your query in the dataset. "
#                     f"Available data from 2025{dates_desc} shows {get_dataset_stats(db_uri)}, "
#                     "with strong stratification (surface warm, deep cold), indicating typical oceanic conditions. "
#                     "Try asking for available dates or broadening your query."
#                 )
#         if visualization_data:
